...
def check_for_win():
    ...
    first_diagonal_win = all([board[i][i] == player_symbol for i in range(size)]) #всичко да е TRUE
...
def place_symbol(position):
    ...
    board[row][col] = players[0][1] #player symbol

    check_for_win() #така сменяме играчите
